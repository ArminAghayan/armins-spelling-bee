...
import { fetchRecentScores, type UserStats, type HofScore } from '@/lib/supabase'
...
export default function StatsModal({ stats, onClose }: Props) {
  ...
  useEffect(() => {
    if (stats?.id) fetchRecentScores(stats.id).then(setRecentGames)
  }, [stats?.id])
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
          ...
          <div>
            <div style={{ fontSize: '17px', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.3px' }}>{displayName}</div>
            <div style={{ fontSize: '11px', color: 'var(--text5)', marginTop: '2px' }}>Player Stats</div>
          </div>
        </div>

        {/* Top row — big highlights */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
          <StatCard
            icon={<IconTrophy size={13} stroke={2} />}
            label="Ranked Best"
            value={s?.ranked_high_score || 0}
            sub="Highest ranked score"
            accent
          />
          <StatCard
            icon={<IconFlame size={13} stroke={2} />}
            label="Best Streak"
            value={s?.highest_streak || 0}
            sub="All-time highest"
            accent
          />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
          <StatCard
            icon={<IconTarget size={13} stroke={2} />}
            label="Accuracy"
            value={`${accuracy}%`}
            sub={`${correct} / ${attempted} words`}
          />
          <StatCard
            icon={<IconLetterCase size={13} stroke={2} />}
            label="Longest Word"
            value={s?.longest_word ? s.longest_word.length : 0}
            sub={s?.longest_word ? `"${s.longest_word}"` : 'No word yet'}
          />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '20px' }}>
          <StatCard
            icon={<IconThumbUp size={13} stroke={2} />}
            label="Wins"
            value={wins}
          />
          <StatCard
            icon={<IconThumbDown size={13} stroke={2} />}
            label="Losses"
            value={losses}
          />
          <StatCard
            icon={<IconChartPie size={13} stroke={2} />}
            label="Win Rate"
            value={`${winRate}%`}
            sub={totalGames > 0 ? `${totalGames} games` : 'No games yet'}
          />
        </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          {modeEntries.length === 0 ? (
            <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px', textAlign: 'center', color: 'var(--text5)', fontSize: '12px' }}>
              No multiplayer wins yet — keep playing!
            </div>
          ) : (
              ...
              {modeEntries.map(([mode, count]) => {
                ...
                return (
                  <div key={mode} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ minWidth: '56px', fontSize: '12px', fontWeight: 600, color: 'var(--text3)' }}>
                    ...
                    <div style={{ minWidth: '24px', fontSize: '12px', fontWeight: 700, color: '#f59e0b', fontFamily: 'Space Mono, monospace', textAlign: 'right' }}>
                      {count}
                    </div>
                  </div>
                )
        ...
        <div>
          ...
          {recentGames.length === 0 ? (
            <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px', textAlign: 'center', color: 'var(--text5)', fontSize: '12px' }}>
              No ranked games yet — play a ranked game to see history!
            </div>
          ) : (
